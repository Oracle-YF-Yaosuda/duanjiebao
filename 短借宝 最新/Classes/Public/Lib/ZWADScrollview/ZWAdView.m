...
-(void)loadAdDataThenStart
{
    ...
    {
        UIImageView *adImageView=[[UIImageView alloc]initWithFrame:CGRectMake((i+1)*self.adScrollView.bounds.size.width, 0, self.adScrollView.bounds.size.width,self.adScrollView.bounds.size.height/2)];
        adImageView.tag=i;
        adImageView.userInteractionEnabled=NO;
        adImageView.contentMode = UIViewContentModeScaleAspectFill;
        adImageView.clipsToBounds = YES;
        adImageView.image = [UIImage imageNamed:self.adDataArray[i]];
        ...
        UILabel *lbl = [[UILabel alloc] initWithFrame:CGRectMake((i+1)*self.adScrollView.bounds.size.width, self.adScrollView.bounds.size.height/2, self.adScrollView.bounds.size.width,self.adScrollView.bounds.size.height/2)];
        lbl.alpha = 0.8;
        lbl.backgroundColor = [UIColor darkGrayColor];
        lbl.textAlignment = NSTextAlignmentLeft;
        lbl.font = [UIFont systemFontOfSize:15];
        lbl.textColor = [UIColor whiteColor];
        lbl.text = self.adTextArray[i];
        [self.adScrollView addSubview:lbl];
    }
